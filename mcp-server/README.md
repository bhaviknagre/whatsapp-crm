# WhatsApp CRM MCP server

A [Model Context Protocol](https://modelcontextprotocol.io) server for
[WhatsApp CRM](https://github.com/bhaviknagre/whatsapp-crm) — a
self-hosted CRM for WhatsApp. It lets MCP clients (Claude Desktop,
...
WhatsApp CRM instance — this server just exposes the API as MCP tools.

## Prerequisites

1. A running WhatsApp CRM instance (your own self-hosted deploy).
...
2. **API-key scopes.** Whatever the guards allow, your WhatsApp CRM
   instance still enforces the key's scopes. A call without the right scope
